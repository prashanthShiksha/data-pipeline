...
class MentoringEventSource extends SourceFunction[Event] {
  ...
  override def run(ctx: SourceContext[Event]): Unit = {
    ...
    //    ctx.collect(new Event(JSONUtil.deserialize[java.util.Map[String, Any]](EventsMock.S_ATT_CREATE), 0, 0))
    //    ctx.collect(new Event(JSONUtil.deserialize[java.util.Map[String, Any]](EventsMock.S_ATT_UPDATE), 0, 0))
    //    ctx.collect(new Event(JSONUtil.deserialize[java.util.Map[String, Any]](EventsMock.S_ATT_DELETE), 0, 0))
    //    ctx.collect(new Event(JSONUtil.deserialize[java.util.Map[String, Any]](EventsMock.OrgMenRating_CREATE), 0, 0))
    //    ctx.collect(new Event(JSONUtil.deserialize[java.util.Map[String, Any]](EventsMock.OrgMenRating_UPDATE), 0, 0))
