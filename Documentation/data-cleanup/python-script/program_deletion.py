...
# ----------------------------
# Load Configuration from unified-pipeline.conf
# ----------------------------
_conf_path = os.environ.get(
    "UNIFIED_PIPELINE_CONF",
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "../../../../unified-pipeline.conf")
)
_conf = ConfigFactory.parse_file(_conf_path)
_pd   = _conf["data-cleanup"]["program-deletion"]
_common = _conf["data-cleanup"]["common"]

# ----------------------------
# Setup Logging
# ----------------------------
LOG_DIR = _common["log-path"]
...
# API Endpoints
# ----------------------------
LOGIN_URL  = _pd["login-url"]
DB_FIND_URL = _pd["db-find-url"]
USERNAME   = _pd["username"]
PASSWORD   = _pd["password"]
ORIGIN     = _pd.get("origin", "")
...
if __name__ == "__main__":
    ...
    list_of_required_program_ids = list(_pd.get("program-ids", []))
