...
import org.shikshalokam.job.dashboard.creator.task.{ObservationMetabaseDashboardConfig, ObservationMetabaseDashboardTask}
...
class ObservationMetabaseDashboardFunctionTestSpec extends BaseTestSpec{
  ...
  def initialize() {
    ...
    when(mockKafkaUtil.kafkaStringSink(jobConfig.inputTopic)).thenReturn(new GenerateObservationMetabaseDashboardSink)
  }
